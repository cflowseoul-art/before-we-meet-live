...
export const VALUE_TO_TAGS: Record<string, string[]> = {
  "서운함이 생기면 그 자리에서 바로 푸는 소통": ["표현", "직설", "열정"],
  "감정을 가라앉히고 혼자 정리할 충분한 시간": ["숙고", "인내", "신중"],
  "모든 일상과 비밀을 투명하게 공유하는 관계": ["밀착", "공유", "투명"],
  "서로의 사생활과 개인적 공간을 존중하는 독립성": ["자율", "독립", "존중"],
  "말하지 않아도 마음을 알아주는 따뜻한 공감": ["공감", "감성", "다정"],
  "문제를 명확히 짚어주고 해결해주는 이성적 조언": ["객관", "이성", "해결"],
  "바쁜 와중에도 연인을 최우선으로 두는 헌신": ["헌신", "희생", "깊이"],
  "나의 커리어와 자기계발을 1순위로 두는 삶": ["성취", "성장", "주관"],
  "익숙하고 편안한 루틴 속에서 느끼는 안정감": ["안정", "평화", "지속"],
  "늘 새로운 곳을 탐험하고 도전하는 짜릿한 일상": ["도전", "모험", "변화"]
};
...
//   "소중한 사람을 위한 헌신": "헌신",
// };
// 2. 가치관 키워드 매핑 (UI 노출용 핵심 4글자 키워드)
...
// 3. 아우라 및 리포트 설명 (양극단의 매력을 살린 톤앤매너)
export const AURA_MAP: Record<string, { aura: string; description: string; gradient: string }> = {
  "즉각표현": { aura: "Solar Flare", description: "숨김없는 솔직함이 상대의 마음을 빠르게 녹일거에요", gradient: "from-orange-400 to-red-500" },
  "신중숙고": { aura: "Deep Forest", description: "고요한 깊이가 관계의 무게중심을 단단히 잡아줄 수 있어요", gradient: "from-emerald-600 to-teal-700" },
  "정서밀착": { aura: "Warm Gravity", description: "모든 것을 함께하고 싶은 강한 중력이 흐르는 분이네요", gradient: "from-rose-400 to-pink-500" },
  "개인자율": { aura: "Slow Orbit", description: "서로의 궤도를 지켜주는 성숙한 거리감을 선호하는 분이군요", gradient: "from-sky-400 to-indigo-500" },
  "감성공감": { aura: "Echo Heart", description: "타인의 아픔에 주파수를 맞추는 다정함을 가진 분이에요", gradient: "from-pink-300 to-rose-400" },
  "논리객관": { aura: "Crystal Mind", description: "현명한 판단으로 관계의 방향을 제시하는 나침반같은 분이에요", gradient: "from-blue-400 to-slate-500" },
  "관계우선": { aura: "Infinite Anchor", description: "사랑하는 사람을 위해 나를 기꺼이 내어주는 헌신을 보여줘요", gradient: "from-amber-400 to-orange-500" },
  "성취우선": { aura: "Lone Star", description: "스스로 빛나는 별이 되어 서로에게 영감이 되어요", gradient: "from-violet-500 to-fuchsia-600" },
  "안정지향": { aura: "Still Water", description: "변함없는 일상의 평온함으로 행복을 찾아요", gradient: "from-teal-300 to-cyan-400" },
  "변화주도": { aura: "Storm Rider", description: "지루할 틈 없는 새로움으로 관계에 활력을 불어넣어요", gradient: "from-yellow-400 to-amber-500" }
};
...
// 4-1. 대화 가이드 (Selection Based: 가벼운 밸런스 게임형)
export const CHEAT_SHEET: Record<string, string> = {
  "즉각표현": "밤 11시, 데이트 후 집 가는 길에 말다툼이 터졌다면? '밤새워서라도 지금 당장 푼다' VS '피곤하니 일단 자고 내일 얘기한다'",
  "신중숙고": "상대에게 서운한 게 생겼을 때 내 태도는? '표정에서 바로 티가 난다' VS '머릿속으로 정리될 때까지 일단 포커페이스'",
  "정서밀착": "데이트 중 내 친구한테 번개 제안이 왔다면? '당연히 연인이랑 같이 간다' VS '데이트 중이니 나중에 따로 보겠다고 한다'",
  "개인자율": "꿈에 그리던 '완벽한 독조(혼자 술 마시기)' 중 연인의 영상통화? '기쁘게 받아서 같이 즐긴다' VS '혼자만의 시간이니 나중에 하겠다고 한다'",
  "감성공감": "연인이 회사에서 상사한테 깨지고 울면서 전화했다면? '누구야? 같이 욕해주며 편든다' VS '왜 깨졌는지 상황 파악 후 피드백해준다'",
  "논리객관": "중요한 결정을 앞두고 연인에게 듣고 싶은 말은? '네가 뭘 하든 무조건 믿어' VS '리스크는 이거니까 신중하게 결정해'",
  "관계우선": "기다렸던 대작 게임 출시일과 연인과의 번개 데이트가 겹치면? '당연히 연인 보러 나간다' VS '오늘은 집에서 게임하며 쉬겠다고 한다'",
  "성취우선": "기념일 당일, 갑작스러운 야근 지시가 내려왔다면? '취소하고 일에 집중한다' VS '욕먹더라도 미루고 데이트하러 간다'",
  "안정지향": "이번 주말 데이트 코스를 짠다면? '검증된 단골집에서 편안하게' VS '한 번도 안 가본 낯선 동네에서 탐험하기'",
  "변화주도": "길 가다 맛있어 보이는 식당을 발견했다면? '예약한 곳 취소하고 바로 들어간다' VS '예약한 곳이 있으니 다음에 오자고 한다'"
};


// 4-2. [V13.0] 상황 가이드 2: 관계의 '본체'를 건드리는 일들
export const CHEAT_SHEET_2: Record<string, string> = {
  "즉각표현": "연인의 폰에 모르는 사람의 다정한 카톡이 온 걸 봤다면? '그 자리에서 바로 물어본다' VS '지켜보다가 확실한 증거가 잡히면 말한다'",
  "신중숙고": "크게 싸우고 냉전 중일 때 내 방식은? '상대가 먼저 말 걸 때까지 기다린다' VS '내가 먼저 장문의 카톡이라도 보낸다'",
  "정서밀착": "꿈꾸던 이직 자리가 연인과 차로 4시간 거리라면? '커리어보다 가까이 있는 게 중요하니 포기' VS '장거리 연애를 하더라도 일단 간다'",
  "개인자율": "연인의 취미가 나랑 너무 안 맞는데 같이 하자고 졸졸 따라다닌다면? '사랑하니까 억지로라도 같이 한다' VS '취미는 각자 즐기자고 확실히 선 긋는다'",
  "감성공감": "연인이 큰 실수를 해서 자괴감에 빠져 있다면? '말없이 꽉 안아주며 토닥인다' VS '정신 차리라고 따끔하게 한 마디 해준다'",
  "논리객관": "내가 잘못해서 싸웠을 때 내가 원하는 화해법은? '맛있는 거 먹으며 기분부터 풀기' VS '내가 뭘 잘못했는지 논리적으로 정리해서 사과하기'",
  "관계우선": "로또 1등에 당첨됐다면? '당장 연인에게 말하고 같이 계획을 짠다' VS '일단 숨기고 나중에 상황 봐서 말한다'",
  "성취우선": "성공을 위해 1년간 해외 연수를 가야 한다면? '연인에게 같이 가자고 하거나 기다려달라고 한다' VS '관계에 걸림돌이 될 것 같으니 정리한다'",
  "안정지향": "10년 뒤 우리의 모습은? '지금처럼 평온하고 익숙한 모습' VS '계속해서 새로운 목표를 찾아 변화하는 모습'",
  "변화주도": "갑자기 전 재산을 털어 세계 일주를 가자는 연인의 제안? '미쳤어? 절대 안 돼' VS '한 번뿐인 인생, 같이 가보자!'"
};
...
// 5. 온도계 분류 (Warm vs Cool)
export const COOL_VALUES = ["신중숙고", "개인자율", "논리객관", "성취우선", "변화주도"];
export const WARM_VALUES = ["즉각표현", "정서밀착", "감성공감", "관계우선", "안정지향"];

export const WARM_CHARMS = ["다정다감", "세심한 배려", "예쁜 말투", "밝은 에너지"];
export const COOL_CHARMS = ["깊은 가치관", "매력적 외모", "프로페셔널", "좋은 분위기"];
